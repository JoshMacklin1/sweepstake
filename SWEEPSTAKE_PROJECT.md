...
48. **Elimination-badge timing** — First Casualty / Wiped Out now time group elimination to when a team is **mathematically locked into 4th** (can't qualify via top-2 or best-third), not just their 3rd group game. Added `GROUP_ASSIGNMENTS` to `scoring.js`. Points-only test (never false-flags; may lag a GD-only lock-in by a game).
