...
| `deriveTeamHistory(matches)` | **Per-team** cumulative sweepstake-points history (Teams-table sparklines). Recomputes each team's authoritative total (`deriveGroupPts` + `deriveStages`, same swPts formula the table shows) **as of each finished match** (later results excluded so no team is credited for a future draw), pushing a shared frame for all 48 teams whenever any total changes. Every real scoring event is its own step — group-qualification bonus (+) / group-elim penalty (−), plus each knockout round reached. Endpoints equal the displayed swPts. All series share one timeline (equal length). Memoized in `TeamsLeagueTab`. **Transient-dip guard:** `deriveStages` ranks best-3rd qualifiers "as it stands", so a 3rd-placed team can provisionally flip eliminated↔qualified mid-group; a post-pass clamps each series to its running max up to the point it settles at its final value (erasing recovering dips) while preserving one genuine terminal GROUP_ELIM drop at its real frame |
