...
A second sweepstake covering **Europe's five biggest leagues** — the
Premier League, La Liga, Serie A, Bundesliga and Ligue 1 — plus the
**Champions League** as an overlay gamble, is being built alongside the WC
app. (It started as PL + Championship; the four continental top flights were
added, then the Championship was dropped and the Champions League added.)

| File | Purpose |
|------|---------|
| `league.html` | The league app shell — **a structural clone of the WC app's UI**, adapted to the league format. **Bottom nav** — now **identical to the WC app**: Home · League · Scores · The Race · More, using the WC app's exact nav icon SVGs and active-pill styling. On **desktop (≥768px)** this bottom nav is replaced by a WC-style **left sidebar** (`Sidebar`/`SidebarBtn`) that carries **all** navigation: branded logo, a search pill, then Home · **Players · Teams · Leagues · UCL** (League's inline top-bar toggle is flattened into direct sidebar destinations, so no nav bar renders inside the content window) · Scores · The Race, with **Notifications** (bell + unread badge) and **More** pinned at the bottom. Unlike the WC desktop build, **search + notifications are kept** — moved into the sidebar rather than dropped. The content-column `Header` collapses to just the simulate-season scrubber on desktop. `App` tracks an `isDesktop` flag (resize-listener) and, while an in-league `App` is mounted, lifts the global 480px `#root` cap to a left-aligned 1020px (sticky 260px sidebar + ~760px content column, flush to the viewport's left edge like the WC app); the group gate/lobby keep the narrow 480px column. The **League** tab carries an inline top-bar toggle (`leagueView`) with four destinations: **Players** (the sweeps standings), **Teams** (every owned team ranked by sweepstake points — `TeamsLeagueTab`/`TeamLeagueRow`, crest + sparkline + pts + 7-day position pill, tap → team card), **Leagues** (`LeaguesTab` — a horizontally-scrollable pill row of the five **domestic** leagues from `LEAGUE_COMPETITIONS` (cup type filtered out), above the selected competition's table via `TablesTab comp={…}`), and **UCL** (`ChampionsLeagueTab`). The UCL tab has its own **League phase | Knockouts** sub-nav (mirroring the WC Groups/Bracket split): League phase = the 36-team CL table with **dotted cut-lines** after 8th (→ last 16) and 24th (→ eliminated) via `TablesTab`'s `thresholds` prop, then the knockout play-offs as a **vertical Scores-style fixture list below the table** (`CLPlayoffsRow` → `HomeMatchRow` per leg); Knockouts = the R16→final `CLBracket` only. `CLTie` is the shared tie box (crest + owner + aggregate, winner highlighted) used by both the bracket and the play-off row. `CLBracket` reuses the WC app's `KnockoutBracket` layout (round columns of absolute-positioned tie boxes spaced so the tree reads without connector lines) adapted to two-legged aggregate ties + club crests, fed by `deriveCLBracket`. (Summer qualifying rounds are out of scope — non-owned clubs, not in the tracked data.) **The Race** tab ports the WC `BumpChart` (positions over time) + a "Watch the Race" button opening the animated `BarRaceModal`, fed by `deriveLeagueHistory` (frames are calendar days; no knockout/elimination layers). **Home** mirrors the WC HomeTab section-for-section on a 7-day rhythm: Currently Leading hero, Top of the Table (mini rows + sparklines + pills), Recent Results, Risers & Fallers, New Accolades (`computeNewLeagueBadges` diff), Up Next. **Pop-up cards**: `SnapSheet` (ported drag/snap bottom sheet, 50vh↔92vh, desktop windowed card) hosting `PlayerCardContent` (stat boxes, points-over-time sparkline, teams, accolades, results with 🏅/⚠️ season-bonus milestone rows, upcoming) and `TeamCardContent` (owner card with `playerColor` initial circle, points + season/position boxes, results with per-match pts). Cards swap in place (team↔owner) with **browser-history integration** (back button closes, Escape too). **Sweeps rows = WC PlayerRow anatomy** (rank circle, badges, crest row, sparkline, weekly pill); tapping opens the player card (no inline expansion). Rules & Info is a **pop-over via the More menu** (like the WC app), which also holds the replay toggle and group switch. **Player-first display rule** everywhere: owner name emphasised, club as muted context. Replay scrubber sits under the header when active. **Top bar mirrors the WC app**: a search pill (`LeagueSearchOverlay` — searches players + teams, with recents, opens the card on select) and a notification bell (`LeagueNotificationsDrawer` — season-outcome milestones, leader changes and accolade awards; **deliberately no per-match results**, dropping from the top with an unread badge driven by a per-device last-seen timestamp). The feed is built by the shared `buildLeagueNotifItems(matches, ranked, badges, anchorIso)` helper — used by both the drawer and the App's unread-count `useMemo`, so the bell badge and the drawer always agree; accolades come from `computeNewLeagueBadges` (7-day diff) dated to the player's latest finished match for a stable timestamp. Loads `league-scoring.js?v=N` — same cache-bust rule as the WC pair (currently `?v=20`). `fetchMatches` requests one `/competitions/{code}/matches` per key in `LEAGUE_COMPETITIONS` (five leagues + the Champions League) in parallel, per-competition try/catch so one league failing doesn't blank the rest; a total outage still surfaces an error. The **Rules** pop-over lists all five pot tables, one shared match-points + season-bonus table (they're identical across leagues), and a **Champions League gamble** table; `TeamCardContent`/`PlayerCardContent` show a 🏆 CL line per club. `compLabel(code)` reads `LEAGUE_COMPETITIONS[code].label` (falls back to "League One" for the notional `L1`). `TablesTab`'s `teamName` falls back to the live API's own team names, so every league table renders correctly even for clubs absent from `LEAGUE_TEAMS`. The **Scores** tab uses the WC app's `DayPicker` — a horizontally-scrollable date strip (edge fades, desktop arrows, scroll-snap, auto-centres the selected day); the default day is `anchorIso` clamped to the loaded match range. Any team-vs-team card (`LeagueMatchCard` on Scores/CL play-offs — spaced, or `HomeMatchRow` on Home — compact) is **tappable → `LeagueHeadToHeadCard`** (WC-style H2H modal, Teams\|Players toggle + recent form) and carries a **pot-gap result banner** (`lgResultBanner`: ⚠️ Upset Alert / 🔪 Giant Killer / 🦄 Fairytale, harsher 😱/🌋 for CL knockouts) with a gold border/glow. **Every club shows its pot** — `potTag(id)` → " (P1)" — after its name everywhere (match cards, tables, bracket). The app is branded **Sweepstaker 26/27** with the logo (`LOGO_SRC`) in the header and on the Join/Create landing |
| `league-scoring.js` | Teams (~109, keyed by **numeric API id**, not TLA), pots (3-season weighted composite seeding), points matrices, `LEAGUE_COMPETITIONS` (`PL`, `PD`, `SA`, `BL1`, `FL1` + `CL`). Each domestic league has `games`/`size`/`relegated` (clinch maths) and `type: "top"` (awards WINNER/TOP_4/TOP_7/RELEGATED via `deriveSeasonOutcomes`). **Champions League** is `type: "cup"`: not a draft and not a domestic table — owning a club that's also in the CL is a **pot-relative gamble** laid on top of its league points (`deriveCLProgress` → furthest stage reached; `CL_GAMBLE[stage][pot]` → points, negative for a high-pot favourite that exits early, upside for a lower-pot overachiever; deep runs credited on reach, early exits only once a later round has finished so nobody's scored "out" prematurely). CL flows through `scoreLeaguePlayers` **and both history passes** (invariant preserved: final sparkline frame == live total). `deriveCLBracket` groups knockout fixtures into two-legged aggregate ties (winner = whoever advanced, robust to away goals/penalties) for the UCL-tab bracket; the CL league phase renders as a table (`lgRegularSeason` treats a cup's non-knockout stages as its "regular season"). **The Championship (`ELC`) was removed** — its teams stay in `LEAGUE_TEAMS` but, with no `LEAGUE_MATCH_PTS`/`LEAGUE_BONUS` entry, fall out of the draw pool and scoring (like the `L1` sides); the history/table passes guard against their replay-only fixtures. **Continental leagues**: La Liga (20)/Serie A (20)/Bundesliga (18)/Ligue 1 (18) rosters are the **real 2025-26 final tables** (football-data.org numeric ids), full squads, with **pots = quartiles by finishing position** (derived from the real replay data — `tools/export-replay-2025.js` → `scratchpad/gen-rosters.js`). No `p25` overrides — keyed straight to 2025-26, so **refresh when flipping `LEAGUE_SEASON` to 2026**. Their `LEAGUE_MATCH_PTS`/`LEAGUE_BONUS` reuse the PL matrix (same competition shape; re-fit per league if desired). `lgIsSettled` counts `AWARDED` alongside `FINISHED`/`IN_PLAY`/`PAUSED`. `deriveLeagueTable` / `deriveSeasonOutcomes` / `scoreLeaguePlayers` / `computeLeagueBadges` / `deriveLeagueHistory` (per-day cumulative player totals for sparklines + The Race, incremental — invariant: final frame == live totals) / `deriveLeagueTeamHistory` (per-team twin of the above, for the Teams-view sparklines; same invariant per team) / `computeWeekRankChange` + `lgWeekWindow` (7-day movement, `anchorIso`-aware so replay works) / `deriveLeagueMatchPts` (per-match points attribution for +pts chips and cards) / `computeNewLeagueBadges` (7-day badge diff for Home). Prefixed `LEAGUE_`/`league`/`lg` — no globals shared with `scoring.js`. `LEAGUE_SEASON` is **2025 for testing; flip to 2026 at launch**. **Per-season pot/league**: the base `LEAGUE_TEAMS` entries are the 2026-27 lineup; the nine teams that changed division between seasons carry a `p25` override with their actual 2025-26 league + pot (West Ham/Wolves/Burnley were Premier League; Coventry/Ipswich/Hull were Championship; Cardiff/Bolton/Lincoln were League One → notional `L1`, never score in the replay). A load-time pass keyed on `LEAGUE_SEASON` applies `p25` so all scoring + UI read the season-correct `.league`/`.pot` — critical because match points are priced by the team's own pot (a wrong pot mis-scores the replay, e.g. Coventry's title run at PL-Pot-4 pricing). `lgTeamMatchPts` returns 0 for a team with no matrix (the `L1` teams). **Groups**: `LEAGUE_GROUPS` mirrors the WC app's five groups (Rodents, Silverstream, Cornwall, Macklins, Caversham; families/bots flattened to plain players). **The league has no Grim Reaper** — that was a World-Cup-only mechanic, removed here; every player (Josh included) is drafted a squad and scores normally. `RODENTS` is **drawn by the engine too** (its old hand-draft held Championship teams that left the pool); every group — and any league created in-app — is drawn by `lgEnsureTeams(group)`: deterministic per group code (mulberry32 seeded by the code), pot-balanced by dealing a pot-interleaved sequence with one continuous round-robin pointer (totals differ by ≤1, pots spread; the non-scoring `L1` teams are left out). The **group gate** (`GroupGate` in `league.html`) is a Join/Create landing: Join resolves a code to a built-in or created group; Create takes a name + player list, draws teams, and persists the group to `localStorage` (`lsw_created`) so its code works on reload |
| `league-data-2025.js` | **Real 2025-26 results for all six competitions** (1,941 matches: PL/PD/SA 380, BL1/FL1 306, CL 189) as a `LEAGUE_REPLAY_DATA` global, injected on demand by replay mode (a .js global, not .json, so it works on file:// previews). Regenerate with `tools/export-replay-2025.js` (browser-run, since the build sandbox can't reach the Worker) |
...
**Deliberately no league-strength multiplier.** Season bonuses are identical
across leagues (a Ligue 1 title scores the same base as a Premier League
title). Scaling by UEFA coefficient was built and reverted on purpose:
weighing which leagues are easier, how many games each plays, and who has
Champions League places is part of the strategic fun of picking, so that
judgement is left to the players rather than baked into the scoring.
...
are the flat "knockout drama" layer instead. The existing CORS Worker
forwards every `/competitions/{code}/…` request unchanged.
...
- **Client** (`league.html`, in `GroupGate`): Create collects league name +
  ...
  players, randomises snake order, closes the invite) + a **Preview app**
  button (any player, 1+ in lobby). Preview deals everyone currently in the
  lobby a random pot-fair squad via `lgPreviewDraw(names, comps, seed)`
  (league-scoring.js — same pot-balancing as `lgEnsureTeams` but restricted
  to the lobby's chosen competitions, re-seeded each press so it re-rolls),
  then hands off to `App` in **preview mode** (`previewMode`/`onExitPreview`
  props): it auto-toggles the last-season replay simulation on mount and
  shows a dismissable "Preview" banner. Nothing is sent to the Worker — the
  real draft state is untouched; **Exit preview** (banner or More → Switch
  group) returns to the live lobby. Lets mates trial the scoring on last
