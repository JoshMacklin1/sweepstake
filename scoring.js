...
function compute24hRankChange(matches, currentRanked) {
  ...
  currentRanked.forEach((p, i) => {
    ...
    // If the player wasn't resolvable 24h ago for any reason (shouldn't
    // normally happen — every player exists in every scorePlayers() call
    // regardless of whether they have points yet — but defensive in case
    // the roster itself ever changes), treat as no change rather than
    // crashing or showing a misleading number.
    result.set(p.name, prevRank !== undefined ? prevRank - i : 0);
  });
  return result;
}
