...
function compute24hRankChange(matches, currentRanked) {
  ...
  // Use finished-only matches for both snapshots so live/in-play matches
  // (which score draw points mid-game) don't skew either side of the delta.
  // currentRanked includes live points in the displayed total, but rank
  // *movement* should compare completed results only — apples-to-apples.
  const finishedOnly = matches.filter(m => m.status === "FINISHED");
  const matches24hAgo = finishedOnly.filter(m => new Date(m.utcDate).getTime() <= cutoffMs);
  const rankedNowFinished = scorePlayers(finishedOnly);
  const ranked24hAgo = scorePlayers(matches24hAgo);

  const rankNowByName = {};
  rankedNowFinished.forEach((p, i) => { rankNowByName[p.name] = i; });
